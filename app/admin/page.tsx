...
export default function AdminPage() {
  ...
  if (!authorized) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <form onSubmit={handlePasswordSubmit} className="bg-white p-6 rounded shadow-md max-w-sm w-full">
          ...
          <button type="submit" className="w-full bg-primary text-white py-2 rounded hover:bg-green-600 transition">
            Submit
          </button>
        </form>
      </div>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">BaseGigs Admin</h1>

      {/* Tabs */}
      <div className="flex space-x-2 mb-6 border-b">
        <button
          onClick={() => setActiveTab('clients')}
          className={`px-6 py-2 font-semibold text-sm rounded-t-lg transition ${
            activeTab === 'clients'
      ...
      {activeTab === 'clients' && (
        <div>
          <h2 className="text-lg font-bold mb-4">Client Subscriptions</h2>
          <input
            type="text"
                        ...
                        {(Object.keys(PLANS) as PlanKey[]).map((key) => (
                          <button
                            ...
                            className={`px-2 py-1 text-sm rounded ${
                              sub?.plan_name === key ? 'bg-primary text-white' : 'bg-gray-200'
                            }`}
                          >
                            {PLANS[key].label}
                          </button>
      ...
      {activeTab === 'seekers' && (
        <div>
          <h2 className="text-lg font-bold mb-4">Gig Seeker Verification</h2>
          <input
            type="text"
            placeholder="Search by name or email"
            value={seekerSearch}
          ...
          <div className="flex space-x-4 mb-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-2 text-sm">
              ...
              <span className="text-green-700">{gigSeekers.filter(s => s.verified).length}</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {filteredSeekers.map((seeker) => {
              ...
              return (
                <div key={seeker.user_id} className={`bg-white border rounded-lg p-4 flex items-center gap-4 ${seeker.verified ? 'border-green-300' : 'border-gray-200'}`}>
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${seeker.verified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
